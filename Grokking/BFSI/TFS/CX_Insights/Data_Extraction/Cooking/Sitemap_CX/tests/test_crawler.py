...
@pytest.fixture
def mock_fetcher():
    with patch('sitemap_crawler.crawler.engine.Fetcher') as MockFetcher:
...
@pytest.fixture
def mock_robots():
    with patch('sitemap_crawler.crawler.engine.RobotsParser') as MockRobots:
...
def test_robots_blocking(config, store, mock_fetcher):
    ...
    with patch('sitemap_crawler.crawler.engine.RobotsParser') as MockRobots:
        ...
        crawler = Crawler(config)
        crawler.process_url("https://example.com/blocked", 1, None)
        
        doc = store.get_document("https://example.com/blocked")
...
def test_excluded_sections(config, store, mock_fetcher, mock_robots):
    crawler = Crawler(config)
    url = "https://example.com/us/en/accounts/login"
    crawler.process_url(url, 1, None)
...
def test_depth_enforcement_general(config, store, mock_fetcher, mock_robots):
    ...
    mock_response = MagicMock(status_code=200, text="""
        <html>
            <body>
                <a href="https://example.com/depth4">Link</a>
            </body>
        </html>
    """, headers={'Content-Type': 'text/html'})
    mock_fetcher.fetch.return_value = (mock_response, None)
    
    crawler.process_url("https://example.com/depth3", 3, None)
    ...
    assert not store.is_url_visited_or_queued("https://example.com/depth4")
...
def test_depth_enforcement_faq_context(config, store, mock_fetcher, mock_robots):
    ...
    crawler.process_url("https://example.com/depth3_faq", 3, None)
    
    # Logic: is_faq_page = True. effective_limit = 6.
    # next_depth = 4. 4 <= 6 is True. Should queue.
    
    assert store.is_url_visited_or_queued("https://example.com/depth4")

def test_resumability(config, store):
    # Add items to queue
    store.queue_url("https://example.com/pending1", 1)
    store.queue_url("https://example.com/pending2", 1)
    ...
    assert item['url'] in ["https://example.com/pending1", "https://example.com/pending2"]
