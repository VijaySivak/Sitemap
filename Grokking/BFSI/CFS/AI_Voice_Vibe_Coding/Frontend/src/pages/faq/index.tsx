...
const FAQ = () => {
  ...
  const categoryIcons: { [key: string]: any } = {
    ...
    Briefcase,
    FileText,
    BookOpen: FileText // Using FileText as fallback for BookOpen
  };
  ...
  const popularFAQs = useMemo(() => {
    return [...faqData.faqs].sort((a, b) => b.views - a.views);
  }, []);
  ...
  const categoryStats = useMemo(() => {
    return faqData.categories.map(category => {
      const categoryFAQs = faqData.faqs.filter(faq => faq.category === category.id);
      const totalViews = categoryFAQs.reduce((sum, faq) => sum + faq.views, 0);
      const avgHelpful = categoryFAQs.reduce((sum, faq) => {
        return sum + (faq.helpful / (faq.helpful + faq.notHelpful)) * 100;
      }, 0) / categoryFAQs.length;
      
      const Icon = categoryIcons[category.icon];
      const gradient = categoryColors[category.color];
      ...
      return {
        ...
        views: totalViews,
        avgHelpful: Math.round(avgHelpful),
        icon: Icon,
        gradient
      };
    });
  ...
  const handleAnalyticsFAQClick = (faqId: string) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        >
          ...
          >
            {showAnalytics ? 'Hide' : 'Show'} Analytics & Popular Questions
          </Button>
        </motion.div>
          ...
          {showAnalytics && (
            ...
            >
              <FAQAnalytics
                ...
                topRatedFAQs={topRatedFAQs}
                categoryStats={categoryStats}
                onFAQClick={handleAnalyticsFAQClick}
              />
            </motion.div>
          )}
        </AnimatePresence>
