...
function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [statsData, faqsData, extData, redundantData] = await Promise.all([
          fetchStats(),
          fetchFaqs(),
          fetchExternalStats(),
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {externalStats && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
             ...
             {/* Sensitive Domains + Redundant Content - Same height as Top Domains */}
             <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex flex-col h-[400px]">
                <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
                  <Shield className="w-5 h-5 text-red-500" />
                  Sensitive Domains Detected
                </h3>
                <div className="overflow-y-auto max-h-[80px] flex-shrink-0">
                  {externalStats.sensitive_domains.length > 0 ? (
                    ...
                    <div className="py-4 flex flex-col items-center justify-center text-slate-400">
                      ...
                      <p className="text-sm">No sensitive domains detected</p>
                    </div>
                  )}
                </div>

                {/* Redundant Content Stats - Below Sensitive Domains */}
                {redundantContent && (
                  <div className="mt-4 pt-4 border-t border-slate-100 flex-1 flex flex-col min-h-0">
                    <h3 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
                      <Copy className="w-5 h-5 text-amber-500" />
                      Redundant Content Stats
                      <span className="ml-auto text-sm font-normal text-slate-500">
                        {redundantContent.total_redundant_blocks} duplicate blocks found
                      </span>
                    </h3>
                    <div className="overflow-y-auto flex-1">
                      {redundantContent.items.length > 0 ? (
                        <ul className="space-y-3">
                          {redundantContent.items.slice(0, 15).map((item, idx) => (
                            <li key={idx} className="p-3 bg-amber-50 border border-amber-100 rounded-lg">
                              <div className="flex justify-between items-start gap-3">
                                <p className="text-sm text-slate-700 flex-1 line-clamp-2" title={item.full_content}>
                                  {item.content_snippet}
                                </p>
                                <span className="flex-shrink-0 bg-amber-200 text-amber-800 px-2 py-1 rounded text-xs font-bold whitespace-nowrap">
                        ...
                        <div className="py-4 flex flex-col items-center justify-center text-slate-400">
                          <Copy className="w-8 h-8 mb-2 opacity-20" />
                          <p className="text-sm">No redundant content detected</p>
                        </div>
                      )}
                    </div>
                  </div>
